...
def check_FuncFilePath(path1, file, opt="Func"):
    ...
    """
    try:
        if os.path.exists(os.path.join(path1, file) + '/config.json') is False | os.path.exists(
                os.path.join(path1, file) + '/main.py') is False:
            return False
        with open(os.path.join(path1, file) + '/config.json') as file_obj:  # 打开config
            contents = file_obj.read()
            contents = json.loads(contents)
            if contents['enable'] is True and contents['funcType'] == opt:  # 技能包启用
                '''
